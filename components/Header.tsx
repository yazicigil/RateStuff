...
export default function Header({ controls }: { controls?: Controls }) {
  ...
  // Ortak: arama + sıralama bloğu (responsive olarak iki kez renderlayacağız)
  function ControlsBlock({ className = '' }: { className?: string }) {
  ...
  return (
    <header className="sticky top-0 z-40 backdrop-blur border-b bg-white/80 dark:bg-gray-900/70 dark:border-gray-800">
      <div className="max-w-5xl mx-auto px-4 py-3 flex flex-col gap-3">
        {/* Üst satır (mobilde: logo solda, sağda tema + giriş/profil) */}
        <div className="flex items-center gap-3">
          <Link href="/" className="text-xl font-bold whitespace-nowrap">RateStuff</Link>

          {/* PC için arama bloğu (tek satır tutmak için burada) */}
          <ControlsBlock className="hidden md:flex flex-1" />
        ...
        {/* Mobil için arama bloğu (ikinci satır) */}
        <ControlsBlock className="md:hidden w-full" />
      </div>
    </header>
  );
}
