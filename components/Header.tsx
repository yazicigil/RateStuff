...
export default function Header({ controls }: { controls?: Controls }) {
  ...
  return (
    <header className="sticky top-0 z-40 backdrop-blur border-b bg-white/80 dark:bg-gray-900/70 dark:border-gray-800">
      {/* mobile: iki satır, desktop: tek satır */}
      <div className="max-w-5xl mx-auto px-4 py-3 flex flex-col gap-3 md:flex-row md:items-center md:gap-3">
        {/* sol: logo */}
        <div className="flex items-center justify-between md:justify-start gap-3">
          <Link href="/" className="text-xl font-bold">RateStuff</Link>

          {/* sağdaki küçük butonlar (mobile üst satır) */}
          <nav className="flex items-center gap-2 md:hidden">
            <ThemePicker theme={theme} onChange={changeTheme} />
            {!loading && !me && (
              <button
                onClick={() => signIn('google', { callbackUrl: '/' })}
                className="p-2 rounded-xl border text-sm dark:border-gray-700 flex items-center gap-2"
            ...
            {me && (
              <>
                <Link
                  href="/me"
                  className="p-1.5 rounded-xl border text-sm dark:border-gray-700"
                  title="Profilim"
                >
                  {me.avatarUrl
                ...
                <button
                  ...
                  className="px-2 py-1.5 rounded-xl border text-sm dark:border-gray-700"
                  title="Çıkış yap"
                  type="button"
                >
                  Çıkış
        ...
        {/* orta: arama + sıralama (mobile alt satır, desktop ortada) */}
        {controls && (
          <div className="flex w-full items-center gap-2 md:mx-auto md:max-w-xl">
            <div className="relative flex-1">
              <input
                value={controls.q}
                onChange={(e) => controls.onQ(e.target.value)}
                placeholder="ara ( / )"
                className="w-full border rounded-xl px-3 py-2 pr-8 text-sm dark:bg-gray-800 dark:border-gray-700 dark:text-gray-100 dark:placeholder-gray-400"
              />
              {!!controls.q && (
                <button
            ...
            <select
              ...
              className="border rounded-xl px-2 py-2 text-sm dark:bg-gray-800 dark:border-gray-700 dark:text-gray-100 w-[110px] md:w-auto"
            >
        ...
        {/* sağ: butonlar (sadece desktop görünür) */}
        <nav className="hidden md:flex items-center gap-2 md:ml-auto">
          <ThemePicker theme={theme} onChange={changeTheme} />
          {!loading && !me && (
            <button
              onClick={() => signIn('google', { callbackUrl: '/' })}
...
function ThemePicker({ theme, onChange }: { theme: ThemePref; onChange: (t: ThemePref)=>void }) {
  return (
    <select
      value={theme}
